...
from codegraph.api.schemas import MessageOut, ThreadRootOut
from codegraph.db import get_session
from codegraph.query import messages as q
...
async def list_(
    ...
    threads = await q.list_threads(
        session, node_id=node_id, agent_id=agent_id, limit=limit, offset=offset
    )
    return {
        "threads": [
            ThreadRootOut(
